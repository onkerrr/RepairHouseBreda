<div>
    ...
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-zinc-900 dark:text-white">Modellen Beheer</h2>
            <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-1">Beheer alle apparaat modellen</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden hover:shadow-md transition-shadow">
                <div class="bg-zinc-900 dark:bg-zinc-950 p-4">
                    <div class="flex items-center justify-between">
                        <span class="text-sm font-medium text-zinc-400">{{ $model->brand->name }}</span>
                        <span class="text-sm font-medium px-3 py-1 rounded {{ $model->is_active ? 'bg-green-500/20 text-green-300' : 'bg-red-500/20 text-red-300' }}">
                ...
                <div class="p-6">
                    ...
                    <div class="flex items-center gap-2 text-sm text-zinc-600 dark:text-zinc-400 mb-4">
                        ...
                        {{ $model->parts_count }} {{ $model->parts_count === 1 ? 'onderdeel' : 'onderdelen' }}
                    </div>

                    <div class="flex gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-700">
                        <button wire:click="show({{ $model->id }})" class="flex-1 px-3 py-2 bg-zinc-100 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-600 transition-colors text-sm font-medium">
                            Bekijken
                        </button>
                        <button wire:click="edit({{ $model->id }})" class="flex-1 px-3 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-md hover:bg-zinc-700 dark:hover:bg-zinc-100 transition-colors text-sm font-medium">
                            Bewerken
                        </button>
                        <button wire:click="confirmDelete({{ $model->id }})" class="px-3 py-2 bg-red-600 dark:bg-red-500 text-white rounded-md hover:bg-red-700 dark:hover:bg-red-600 transition-colors">
        ...
        @empty
            <div class="col-span-full text-center py-12 bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700">
                ...
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-white mb-2">Geen modellen gevonden</h3>
                <p class="text-zinc-600 dark:text-zinc-400 mb-4">Er zijn nog geen modellen aangemaakt.</p>
            </div>
        @endforelse
    </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-700">
                    <h3 class="text-xl font-bold text-zinc-900 dark:text-white">
                        {{ $modelId ? 'Model Bewerken' : 'Nieuw Model' }}
                    </h3>
                </div>
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" wire:model="is_active" id="is_active_model"
                        ...
                        <label for="is_active_model" class="ml-2 text-sm font-medium text-zinc-700 dark:text-zinc-300">
                            Actief
                        </label>
                    </div>
                </div>
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-zinc-600 dark:text-zinc-400 mt-1">
                                Weet je zeker dat je dit model wilt verwijderen? Alle gerelateerde onderdelen worden ook verwijderd.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-zinc-200 dark:border-zinc-700">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-sm text-zinc-600 dark:text-zinc-400 mb-1">{{ $selectedModel->brand->name }}</p>
                            <h3 class="text-xl font-bold text-zinc-900 dark:text-white">
                                {{ $selectedModel->name }}
                            </h3>
                            <span class="inline-block mt-2 text-sm font-medium px-3 py-1 rounded {{ $selectedModel->is_active ? 'bg-green-500/20 text-green-700 dark:text-green-300' : 'bg-red-500/20 text-red-700 dark:text-red-300' }}">
                                {{ $selectedModel->is_active ? 'Actief' : 'Inactief' }}
                            </span>
                        </div>
                ...
                <div class="p-6 space-y-6">
                    @if($selectedModel->description)
                        <div>
                            ...
                            <p class="text-zinc-900 dark:text-white">{{ $selectedModel->description }}</p>
                        </div>
                    @endif

                    <div>
                        <h4 class="text-sm font-medium text-zinc-600 dark:text-zinc-400 mb-3">Onderdelen ({{ $selectedModel->parts_count }})</h4>
                        @if($selectedModel->parts->count() > 0)
                            <div class="space-y-2">
                                @foreach($selectedModel->parts as $part)
                                    <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
                                        <div class="flex-1">
                                            <p class="font-medium text-zinc-900 dark:text-white">{{ $part->name }}</p>
                                            <p class="text-sm text-zinc-600 dark:text-zinc-400">SKU: {{ $part->sku }} | €{{ number_format($part->price, 2, ',', '.') }}</p>
                                        </div>
                                        <div class="text-right">
                                            <p class="text-sm font-medium {{ $part->isLowStock() ? 'text-amber-600 dark:text-amber-400' : 'text-zinc-900 dark:text-white' }}">
                                                Voorraad: {{ $part->stock }}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        @else
                            <p class="text-zinc-500 dark:text-zinc-400 text-sm italic">Geen onderdelen gevonden</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="p-6 border-t border-zinc-200 dark:border-zinc-700 flex justify-end gap-3">
                    ...
                    <button wire:click="edit({{ $selectedModel->id }})" type="button" 
                        class="px-4 py-2 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 rounded-md hover:bg-zinc-700 dark:hover:bg-zinc-100 transition-colors font-medium">
                        Bewerken
                    </button>
                </div>
